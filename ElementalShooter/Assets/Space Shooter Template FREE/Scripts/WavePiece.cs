...
public class WavePiece : MonoBehaviour
{
    ...
    private void Start()
    {
        //StartCoroutine(CreateEnemyWave(Vector3.zero)); 
    }

    public IEnumerator CreateEnemyWave( Vector3 wavePosition, float incrementX,float positionY,int countPerLine, UnityAction onDeath) //depending on chosed parameters generating enemies and defining their parameters
    {
        for (int i = 0; i < countPerLine; i++)
        {
            GameObject newEnemy = Instantiate(enemy, enemy.transform.position, Quaternion.identity);
            Vector3 NewWavePosition = new Vector3(wavePosition.x+i*incrementX,positionY,wavePosition.z);
            ...
            followComponent.SetPath(NewWavePosition);
            BasicEnemy enemyComponent = newEnemy.GetComponent<BasicEnemy>();
            enemyComponent.OnDeath.AddListener(onDeath);
        ...
        if (testMode)       //if testMode is activated, waiting for 3 sec and re-generating the wave
        {
            ...
            StartCoroutine(CreateEnemyWave( wavePosition, incrementX, positionY, countPerLine, onDeath));
        }
        //else if (!Loop)
        //    Destroy(gameObject);
    }
